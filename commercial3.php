<?php include 'dbCon.php';
   ...
   // $typeofStand=mysqli_real_escape_string($con,$_POST['typeofStand']);
   //  $natureOfDev=mysqli_real_escape_string($con,$_POST['natureOfDev']);
     //$occupationApplicants=mysqli_real_escape_string($con,$_POST['occupationApplicants']);
     //$otherInfo=mysqli_real_escape_string($con,$_POST['otherInfo']);
     //$otherCommStands=mysqli_real_escape_string($con,$_POST['otherCommStands']);
    // $cellphone=mysqli_real_escape_string($con,$_POST['cellphone']);
    // $dependents=mysqli_real_escape_string($con,$_POST['dependents']);
    // $yearsOfRes=mysqli_real_escape_string($con,$_POST['yearsOfRes']);

    //finally, let's store our posted values in the session variables
if(((isset($_SESSION['name']))&&(!empty($_SESSION['name'])))||((isset($_SESSION['surname']))&& (!empty($_SESSION['surname'])))||((isset($_SESSION['address']))&& (!empty($_SESSION['address'])))||((isset($_SESSION['idNum']))&& (!empty($_SESSION['idNum'])))||((isset($_SESSION['dob']))&& (!empty($_SESSION['dob'])))||((isset($_SESSION['prevStand']))&& (!empty($_SESSION['prevStand'])))||((isset($_SESSION['income']))&& (!empty($_SESSION['income'])))||((isset($_POST['typeofStand']))&& (!empty($_POST['typeofStand'])))||((isset($_POST['natureOfDev']))&& (!empty($_POST['natureOfDev'])))||((isset($_POST['amtOfCapital']))&& (!empty($_POST['amtOfCapital'])))||((isset($_POST['otherInfo']))&& (!empty($_POST['otherInfo']))) ||((isset($_POST['otherCommStands']))&&(!empty($_POST['otherCommStands']))))
{

    $_SESSION['typeofStand']=mysqli_real_escape_string($conn,$_POST['typeofStand']);
    $_SESSION['natureOfDev']=mysqli_real_escape_string($conn,$_POST['natureOfDev']);
    $_SESSION['amtOfCapital']=mysqli_real_escape_string($conn,$_POST['amtOfCapital']);
    $_SESSION['otherInfo']=mysqli_real_escape_string($conn,$_POST['otherInfo']);
    $_SESSION['otherCommStands']=mysqli_real_escape_string($conn,$_POST['otherCommStands']);
   
  }else{
        
        echo '<script type="text/javascript"> alert("cannot access this page without filling first page ");</script>';
        header("location: commercial2.php");//redirecting to first page);
    }
			...
			<div class="login1">
			  ...
			  <h4> Enter details</h4>
				  <form action="postCom.php" method="post">
					  <input type="text" placeholder="Name of Referee" name="nameOfReferee" required>
					  <input type="text" placeholder="Address of Referee" name="addressOfReferee" required/>
                      <input type="text" placeholder="Contact Telephone Number" name="contactReferee" required/>
                      <input type="text" placeholder="Relationship with Referee" name="rlnshipwithReferee" required />
                      <p>This application is required to be renewed annually.Failure to do so will result in the removal of the applicant from the waiting list.Any false declaration made in this form will result in the applicant being disqualified from being put on the waiting list.So do solemnly declare that the information contained in this form is true by pressing the Submit Button below.</p>
                      <input type="submit" value="Submit" class="btn1 btn1-primary"/>
